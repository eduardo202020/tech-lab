...
interface ProjectContextType {
  ...
  addProject: (project: Omit<TechProject, 'id' | 'createdAt' | 'updatedAt'>) => void;
...
const INITIAL_PROJECTS: TechProject[] = [
  {
    ...
    description: 'Sistema inteligente de gestión de estacionamientos con sensores IoT y visualización en tiempo real',
    category: 'IoT & Smart Cities',
    technologies: ['React', 'Node.js', 'Arduino', 'Sensores Ultrasónicos', 'MongoDB', 'WebSocket'],
    status: 'active',
    priority: 'high',
  ...
  },
  {
    ...
    description: 'Laboratorio de reconocimiento de imágenes usando deep learning para clasificación automática',
  ...
  },
  {
    ...
    description: 'Sistema de trazabilidad de cadena de suministro usando blockchain para garantizar autenticidad',
...
export function ProjectProvider({ children }: { children: React.ReactNode }) {
  ...
  const addProject = useCallback((newProject: Omit<TechProject, 'id' | 'createdAt' | 'updatedAt'>) => {
    setIsLoading(true);
    setTimeout(() => {
  ...
  const updateProject = useCallback((id: string, updates: Partial<TechProject>) => {
    setIsLoading(true);
    setTimeout(() => {
      setProjects(prev =>
        prev.map(project =>
  ...
  const filterByStatus = useCallback((status: TechProject['status']) => {
    return projects.filter(project => project.status === status);
  }, [projects]);

  const filterByPriority = useCallback((priority: TechProject['priority']) => {
